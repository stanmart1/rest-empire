...
def get_all_packages(
    current_user: User = Depends(require_permission("packages:list")),
...
def create_package(
    ...
    current_user: User = Depends(require_permission("packages:create")),
    db: Session = Depends(get_db)
):
...
def update_package(
    ...
    current_user: User = Depends(require_permission("packages:update")),
    db: Session = Depends(get_db)
):
...
def delete_package(
    package_id: int,
    current_user: User = Depends(require_permission("packages:delete")),
    db: Session = Depends(get_db)
):
...
def approve_activation_payment(
    transaction_id: int,
    current_user: User = Depends(require_permission("transactions:approve")),
    db: Session = Depends(get_db)
):
...
def assign_package_to_users(
    package_id: int,
    user_ids: list[int],
    current_user: User = Depends(require_permission("packages:assign")),
    db: Session = Depends(get_db)
):
